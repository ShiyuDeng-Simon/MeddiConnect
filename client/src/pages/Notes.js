...
import { generateContent } from '../llmServices/gemini';
...
const Notes = () => {
    ...
    if(Array.isArray(message)){
        ...
        generateContent(parsedMessage).then((content) => {
            setMessage(content);
        });
    }
    ...
    function parseConversation(conversation) {
        ...
        const conversationString = conversation.map(obj => `${obj.isUser ? "Patient: " : "Doctor: "}${obj.text}`)
          .join("; ");
    ...
    return (
        ...
        <Container>
            ...
            {noteType === "pre" ?   <TextArea/> :   <AppointmentNotes> {getFormattedText(message || mockNotes)}</AppointmentNotes>}   
        </Container>
        </>
